...
//import com.google.android.cameraview.activity.TakePhotoAct;

/**
 * mark
 * 使用design的时候
 * Binary XML file line # : Error inflating class”
 * 修改当前act 的style  必须要加一个parent
 */
public class PickImageActivity extends UI {
    ...
    public static void start(Activity activity, int requestCode, int from,
                             ...
                             String diyCameraTips, boolean isUseDiyCamera) {
        Intent intent = new Intent(activity, PickImageActivity.class);
        intent.putExtra(Extras.EXTRA_FROM, from);
        intent.putExtra(Extras.EXTRA_FILE_PATH, outPath);
        ...
        intent.putExtra(Extras.EXTRA_DIYCAMERATIPS, diyCameraTips);
        intent.putExtra(Extras.EXTRA_ISUSEDIYCAMERA, isUseDiyCamera);
        activity.startActivityForResult(intent, requestCode);
    }
    ...
    private void pickFromCamera() {
        try {
            ...
            boolean isUseDiyCamera = getIntent().getBooleanExtra(Extras.EXTRA_ISUSEDIYCAMERA,false);
//            if (!isUseDiyCamera) {
...
//            } else {
//                TakePhotoAct.startAct(PickImageActivity.this, REQUEST_CODE_CAMERA, outPath,getIntent().getStringExtra(Extras.EXTRA_DIYCAMERATIPS));
//            }
