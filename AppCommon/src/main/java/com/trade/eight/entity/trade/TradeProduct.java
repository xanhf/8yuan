...
public class TradeProduct extends TradeCommonObj {
    ...
    public String getMyCustomName() {
        String str = ConvertUtil.NVL(getName(), "");
        if (!StringUtil.isEmpty(ConvertUtil.NVL(getWeight(), ""))) {
            str = str + "" + ConvertUtil.NVL(getWeight(), "");
            str = str + "" + ConvertUtil.NVL(getUnit(), "");
        }
        return str;
    }
    ...
    public String getMyCustomNamePrice() {
        ...
        str = str + " (" + ConvertUtil.NVL(getPrice(), "--") + "元/手)";
        return str;
    }
     ...
     * @return
     */
    public int getBuyRateFromList(List<List<TradeProduct>> initList) {
        int up = 0;
        for (List<TradeProduct> list : initList) {
            if (list.get(0).getContract().equals(contract)) {
                for (TradeProduct tradeProduct : list) {
                    String upStr = StringUtil.matcherNumber(tradeProduct.getBuyRate());
                    up += Integer.parseInt(upStr);
                }
                up = (int) (NumberUtil.divide(up, list.size()));
            }
        }
        return up;
    }
