 ...
 * 快速建仓 直播室里头
 */
public class QuickTradeUtilJN extends QuickTradeUtil {
    public static final String TAG = "QuickTradeUtilJN";
     ...
     */
    public QuickTradeUtilJN(BaseActivity c, int type, TradeProduct p, List<List<TradeProduct>> initList) {
...
//        tv_quanCount.setText(null+ "");
    ...
    int getPVoucher(TradeProduct p) {
        ...
        if (voucherRes.isSuccess()) {
            TradeVoucher v = voucherRes.getData();
            return v.getCount();
        }
        return 0;
    }
    ...
    String blance;
    //代金券的返回res
    CommonResponse<TradeVoucher> voucherRes;
...
//                    context.showCusToast(ConvertUtil.NVL(result.getErrorInfo(), "网络连接失败！"));
     ...
     * 获取单个代金券
     */
    class VoucherTask extends AsyncTask<String, Void, CommonResponse<TradeVoucher>> {
        TradeProduct loadP = getTradeProduct();
        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            voucherRes = null;
...
//                context.showNetLoadingProgressDialog(null);
        ...
        @Override
        protected CommonResponse<TradeVoucher> doInBackground(String... params) {
            //取所有代金券
            try {
                voucherRes = TradeHelp.getPVoucherJN(context, getTradeProduct().getProductId());
            } catch (Exception e) {
                e.printStackTrace();
            }
            return voucherRes;
        ...
        @Override
        protected void onPostExecute(CommonResponse<TradeVoucher> result) {
            super.onPostExecute(result);
            if (context == null)
                return;
            if (context.isFinishing())
            ...
            if (result != null) {
                //refresh adapter
                if (result.isSuccess() && loadP.getProductId().equals(getTradeProduct().getProductId())) {
                    tv_quanCount.setText(getPVoucher(null) + "");
                } else {
                    //token过期 避免重复弹框
                    if (ApiConfig.isNeedLogin(result.getErrorCode())) {
//                        if (!StringUtil.isEmpty(result.getErrorInfo())) {
