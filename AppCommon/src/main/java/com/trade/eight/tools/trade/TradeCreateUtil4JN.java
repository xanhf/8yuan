 ...
 * 下单弹窗 农交所
 */
public class TradeCreateUtil4JN extends TradeCreateUtil {
    public static final String TAG = "TradeCreateUtil4JN";
     ...
     */
    public TradeCreateUtil4JN(BaseActivity c, int type, TradeProduct p, List<List<TradeProduct>> initList) {
        ...
        try {
            ...
            View rootView = View.inflate(context, R.layout.dialog_create_order_jn, null);
    ...
    public void showNavDlg() {
        //已经显示过
        if (PreferenceSetting.getBoolean(context, TAG+TradeConfig.getCurrentTradeCode(context)))
            return;
        String msg = context.getResources().getString(R.string.str_msg_jn_fun);
        Dialog d = DialogUtil.showConfirmDlg(context, msg, "关闭", "查看详情", true, null, new Handler.Callback() {
            @Override
            public boolean handleMessage(Message message) {
                ...
                WebActivity.start(context, string, AndroidAPIConfig.URL_TRADE_RULE_JN);
                return false;
            }

        });
        if (d != null) {
            final TextView titleHit = (TextView) d.findViewById(R.id.titleHit);
            String fee = context.getResources().getString(R.string.str_lable_jn_deferred);
            msg = String.format(msg, TradeConfig.isCurrentJN(context) ? TradeConfig.str_trade_jn : TradeConfig.str_trade_hn, fee);
            SpannableString spannableString = new SpannableString(msg);

            try {
                spannableString.setSpan(new StyleSpan(Typeface.BOLD), msg.indexOf(fee), msg.indexOf(fee) + fee.length(),
                        Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            } catch (Exception e) {
                e.printStackTrace();
            }
            titleHit.setText(spannableString);
            titleHit.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 15);
            d.setOnDismissListener(new DialogInterface.OnDismissListener() {
                @Override
                public void onDismiss(DialogInterface dialogInterface) {
                    PreferenceSetting.setBoolean(context, TAG+TradeConfig.getCurrentTradeCode(context), true);
                }
            });
        }
    }
...
//        int screenWidth = displayMetrics.widthPixels;
    ...
    void findViews(Dialog dialog) {
        ...
        if (TradeConfig.isCurrentHN(context)) {
            if (!BaseInterface.isShowHNVoucher) {// 华凝所暂时先隐藏代金券入口
                line_check_usequan.setVisibility(View.GONE);
            }
        }
...
//            setBuyTotalMoneyFee(false);
    ...
    public void submit(final BaseActivity activity) {
        //check at 2017-02-15 代金券之前没有使用 是否使用代金券判断，农交所大厅建仓bug。必须有钱才能使用代金券，快速建仓是可以的
        //先判读是否现金或者代金券是否够用
        if (isQuan) {
    ...
    public void submit(final BaseActivity activity, boolean isQuan) {
        if (activity == null)
            return;
        final UserInfoDao dao = new UserInfoDao(activity);
        TradeProduct product = getTradeProduct();
