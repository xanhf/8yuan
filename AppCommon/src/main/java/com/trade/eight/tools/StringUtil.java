...
public class StringUtil {
    ...
    public static boolean isNickName(Context context, String nikName) {
        if (StringUtil.isEmpty(nikName)) {
            DialogUtil.toast(context, "请输入昵称");
            return false;
        }

        if (getNickLength(nikName) > MAX_LENGTH_nikName) {
            DialogUtil.toast(context, "昵称过长");
            return false;
        }
        return true;
    }
...
//        source = replace(source, "<div>", "");
    ...
    public static String getDeviceInfo() {
        ...
        try {
            str.append("BRAND=" + ConvertUtil.NVL(Build.BRAND, ""));//厂商meizu
            str.append("|MODEL=" + ConvertUtil.NVL(Build.MODEL, ""));//型号mx5
...
//                Log.v(TAG, "getDeviceId utdid="+utdid);
    ...
    public static String getMobinfo(Context context) {
        ...
        try {
            stringBuffer.append("brand_" + ConvertUtil.NVL(Build.BRAND, ""));
