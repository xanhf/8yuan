...
public class SeekBarUtil {
    ...
    public void initSeekBarZhiying(final TradeCommonObj product) {
        if (product == null)
            return;
        //设置显示的最大只止盈点数
        tv_maxZhiying.setText(NumberUtil.moveLast0(product.getMaxStopProfit()) + "点");
        //可设置的最大点数-可设置的最小点数
        double d = product.getMaxStopProfit() - product.getMinStopProfit();
        //乘以放大倍数
        d = NumberUtil.multiply(d, getScale(product));
        //后台设置的移动一个点的值
        double step = NumberUtil.multiply(product.getMinMovePoint(), getScale(product));
        //后台设置的移动一个点的值
        d = NumberUtil.divide(d, step);
        //按照step
        //有小数点的话 需要增加一位
        d = Math.ceil(d);
        int max = (int) d;
...
//        }
    ...
    public void initSeekBarZhisun(final TradeCommonObj product) {
        if (product == null)
            return;
        tv_maxZhisun.setText(NumberUtil.moveLast0(product.getMaxStopLoss()) + "点");
...
//            return 100;
        ...
        int powY = NumberUtil.getPointPow(product.getMinMovePoint()+"");
        if (powY != 0) {
...
//        double realPoint = NumberUtil.add(start, NumberUtil.multiply((p - 1), product.getMinMovePoint()));
        ...
        double p = NumberUtil.divide(NumberUtil.subtract(d, start), product.getMinMovePoint()) + 1;
        //向上取整
...
//                    getMinStopProfitRate 10：表示10%，所以要除以100
            double pMaxPoint = NumberUtil.divide(product.getMinStopProfit(),
                    NumberUtil.divide(product.getMinStopProfitRate(), 100D));

            double realPoint = getPointByPro(product, skb, isZhiying);
            String str = NumberUtil.moveLast0(realPoint) + "点";

            //四舍五入获取到int值
            percent = (int) (NumberUtil.divide(realPoint, pMaxPoint) * 100);
...
//                当前点位/calculatePoint*yk*buyCount
                double money = NumberUtil.multiply(NumberUtil.divide(realPoint, product.getCalculatePoint()),
                        NumberUtil.multiply(product.getYkDouble(), buyCount));

                if (isQuan && !isZhiying) {
                    // 如果是使用券并且是止损的话  设置是0
                    money = 0;
                }
                String strMoney = "(" + NumberUtil.moveLast0(money) + "元)";
                tvValue.setText(str);
    ...
    public void updateSeekMoney(TradeCommonObj product, boolean isQuan, int buyCount) {
        ...
        if (isQuan) {
            ...
            if (realPoint == 0) {
                ...
                money = NumberUtil.multiply(NumberUtil.divide(realPoint, product.getCalculatePoint()),
                        NumberUtil.multiply(product.getYkDouble(), buyCount));
                String strMoney = "(" + NumberUtil.moveLast0(money) + "元)";
    ...
    public static String getZhiying(SeekBar seekBar, TradeCommonObj product) {
        ...
        if (p == 0) {
            ...
            start = product.getMinStopProfit();
            value = NumberUtil.add(start, NumberUtil.multiply((p - 1), product.getMinMovePoint()));
        }
        //镍有可能超过step 0.02 递增
        if (value > product.getMaxStopProfit())
            value = product.getMaxStopProfit();
        return NumberUtil.moveLast0(value);
    }
