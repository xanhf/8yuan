...
package com.trade.eight.camera.utils;
...
public class CameraUtils
{
    ...
    public static Camera.Size getLargestPictureSize(CameraHost host , Camera.Parameters parameters , boolean enforceProfile)
    {
        ...
        for (Camera.Size size : parameters.getSupportedPictureSizes())
        {
            ...
            if (!enforceProfile
                    || (size.height <= host.getDeviceProfile().getMaxPictureHeight() && size.height >= host.getDeviceProfile().getMinPictureHeight()))
            {
                if (result == null)
                {
                    result = size;
    ...
    {
        Camera.Size result = null;
        int maxsize = Utils.getScreenH(context);
        //        Loger.d(" maxsize size----------->" + maxsize+","+DisplayHelper.getScreenWidth());
        //
        //        List<Size> sizes = parameters.getSupportedPictureSizes();
        //        if (Build.DEVICE.contains("HM") && Build.BRAND.toLowerCase().contains("xiaomi")) {
        ...
        //                    result = size;
        //                } else {
        //                    if (Math.max(size.width, size.height) < maxsize) {
        //                        break;
        //                    } else {
        //                        result = size;
        //                    }
        ...
        //            }
        //        }
        //        Loger.d(" result picture size----------->" + result.width + "," + result.height);
        ...
        //        for (Size size : list)
        //        {
        //            if ((size.width > maxsize) && equalRate(size, 1.77f))
        //            {
        //                result = size;
        //                break;
        //            }
