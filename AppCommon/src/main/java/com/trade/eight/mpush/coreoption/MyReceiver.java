...
import com.trade.eight.mpush.android.Notifications;
...
//                    , Toast.LENGTH_SHORT).show();
            NettyResponse<Optional> nettyResponse = new NettyResponse<Optional>();
            nettyResponse.setType(NettyUtil.TYPE_CONNECT);
            EventBus.getDefault().post(new OptionalEvent(nettyResponse));
        } else if (MPushService.ACTION_ORDERNOTIFY.equals(intent.getAction())) {//订单反馈

            EventBus.getDefault().post(new OrderNotifyEvent());

            OrderNotifyData orderNotifyData = (OrderNotifyData) intent.getSerializableExtra(MPushService.EXTRA_ORDERNOTIFY);
            Log.e(TAG, orderNotifyData.toString());
            OrderAndTradeNotifyActivity.startOrderNotifyAct(context,orderNotifyData);

        }else if (MPushService.ACTION_QUOTATION.equals(intent.getAction())) {//行情
//            if(intent.hasExtra()){
//
//            }
            Optional optional = (Optional) intent.getSerializableExtra(MPushService.EXTRA_QUOTATION);
            NettyResponse<Optional> nettyResponse = new NettyResponse<Optional>();
            nettyResponse.setData(optional);
            nettyResponse.setType(NettyUtil.TYPE_QP);
            EventBus.getDefault().post(new OptionalEvent(nettyResponse));
